...
function showMeMagicExterior() {
	...
	if(toggle1) {
		...
		if(cc2%2==0) {
			...
			var geometry = new THREE.CircleGeometry( 0.1, 32,Math.PI/2+rad+ fact1,Math.PI-rad);
			var material = new THREE.MeshBasicMaterial( { color: 0xC0392B,transparent: true, opacity: 0.4} );
			circlee= new THREE.Mesh( geometry, material );
			circlee.position.set((myBallX-0.8)+vertices[0][0], (myBallY+0.3)+vertices[0][1], 0.1)
...
function showMeMagicInterior() {
	...
	if(toggle2) {
		...
		if(cc%2==0) {
			...
			var fact2 = 0;
			for(i=0;i<countOnMe;i++) {
				var geometry = new THREE.CircleGeometry( 0.1, 32,(fact1*Math.PI+(fact2+(rad-Math.PI/2))),rad);
				...
				scopeOther["circle"+i].position.set((myBallX-0.8)+vertices[i][0], (myBallY+0.3)+vertices[i][1], 0.1)
...
function initialiseHelp()
{
	...
	helpContent = helpContent + "<h7>About the experiment</h7>";
	helpContent = helpContent + "<p>The experiment shows polygons of different sides.</p>";
	helpContent = helpContent + "<h7>Animation control</h7>";
	helpContent = helpContent + "<p>The top line has animation controls. There are two states of the experiment.</p>";
	helpContent = helpContent + "<h7>The setup stage</h7>";
	...
	helpContent = helpContent + "<h7>The animation stage</h7>";
...
function initialiseInfo()
{
	...
	infoContent = infoContent + "<h7>About the experiment</h7>";
	infoContent = infoContent + "<p>The experiment shows a container placed under water tap.</p>";
	infoContent = infoContent + "<h7>Pressure</h7>";
...
//toggle button
	...
	//text underneath
	var para2 =showMeMagicText("H6",arrayofShapes[countOnMe],"450px","460px");
    para2.style.cssText = 'position:absolute;top:450px;left:410px;font-family: "Avant Garde", Avantgarde, "Century Gothic", CenturyGothic, "AppleGothic", sans-serif;font-size: 22px;padding: 10px -80px;text-align: center;text-transform: uppercase;text-rendering: optimizeLegibility;';
    var para = showMeMagicText("H1","","270px","250px");
    var para = showMeMagicText("H3","","270px","250px");
	myBall.position.set(myBallX-0.8, myBallY+0.3, myBallZ);
	myBack.position.set(myCenterX, myCenterY, -0.0001);
